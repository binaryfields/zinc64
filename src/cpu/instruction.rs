...
impl Instruction {
    ...
    pub fn decode(cpu: &mut Cpu, opcode: u8) -> Instruction {
        match opcode {
            ...
            // ORA (Oper,X)
            0x01 => Instruction::ORA(Operand::IndirectX(cpu.fetch_byte()), 6),
            // ORA Oper
            0x05 => Instruction::ORA(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // ASL Oper
            0x06 => Instruction::ASL(Operand::ZeroPage(cpu.fetch_byte()), 5),
            // PHP
            0x08 => Instruction::PHP(3),
            // ORA #Oper
            0x09 => Instruction::ORA(Operand::Immediate(cpu.fetch_byte()), 2),
            // ASL A
            ...
            // BPL
            0x10 => Instruction::BPL(Operand::Relative(cpu.fetch_byte() as i8), 2),
            // ORA (Oper),Y
            0x11 => Instruction::ORA(Operand::IndirectY(cpu.fetch_byte()), 5),
            // ORA Oper,X
            0x15 => Instruction::ORA(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // ASL Oper,X
            0x16 => Instruction::ASL(Operand::ZeroPageX(cpu.fetch_byte()), 6),
            // CLC
            ...
            // AND (Oper,X)
            0x21 => Instruction::AND(Operand::IndirectX(cpu.fetch_byte()), 6),
            // BIT Oper
            0x24 => Instruction::BIT(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // AND Oper
            0x25 => Instruction::AND(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // ROL Oper
            0x26 => Instruction::ROL(Operand::ZeroPage(cpu.fetch_byte()), 5),
            // PLP
            0x28 => Instruction::PLP(4),
            // AND #Oper
            0x29 => Instruction::AND(Operand::Immediate(cpu.fetch_byte()), 2),
            // ROL A
            ...
            // BMI
            0x30 => Instruction::BMI(Operand::Relative(cpu.fetch_byte() as i8), 2),
            // AND (Oper),Y
            0x31 => Instruction::AND(Operand::IndirectY(cpu.fetch_byte()), 5),
            // AND Oper,X
            0x35 => Instruction::AND(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // ROL Oper,X
            0x36 => Instruction::ROL(Operand::ZeroPageX(cpu.fetch_byte()), 6),
            // SEC
            ...
            // EOR (Oper,X)
            0x41 => Instruction::EOR(Operand::IndirectX(cpu.fetch_byte()), 6),
            // EOR Oper
            0x45 => Instruction::EOR(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // LSR Oper
            0x46 => Instruction::LSR(Operand::ZeroPage(cpu.fetch_byte()), 5),
            // PHA
            0x48 => Instruction::PHA(3),
            // EOR #Oper
            0x49 => Instruction::EOR(Operand::Immediate(cpu.fetch_byte()), 2),
            // LSR A
            ...
            // BVC
            0x50 => Instruction::BVC(Operand::Relative(cpu.fetch_byte() as i8), 2),
            // EOR (Oper),Y
            0x51 => Instruction::EOR(Operand::IndirectY(cpu.fetch_byte()), 5),
            // EOR Oper,X
            0x55 => Instruction::EOR(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // LSR Oper,X
            0x56 => Instruction::LSR(Operand::ZeroPageX(cpu.fetch_byte()), 6),
            // CLI
            ...
            // ADC (Oper,X)
            0x61 => Instruction::ADC(Operand::IndirectX(cpu.fetch_byte()), 6),
            // ADC Oper
            0x65 => Instruction::ADC(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // ROR Oper
            0x66 => Instruction::ROR(Operand::ZeroPage(cpu.fetch_byte()), 5),
            // PLA
            0x68 => Instruction::PLA(4),
            // ADC #Oper
            0x69 => Instruction::ADC(Operand::Immediate(cpu.fetch_byte()), 2),
            // ROR A
            ...
            // BVS
            0x70 => Instruction::BVS(Operand::Relative(cpu.fetch_byte() as i8), 2),
            // ADC (Oper),Y
            0x71 => Instruction::ADC(Operand::IndirectY(cpu.fetch_byte()), 5),
            // ADC Oper,X
            0x75 => Instruction::ADC(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // ROR Oper,X
            0x76 => Instruction::ROR(Operand::ZeroPageX(cpu.fetch_byte()), 6),
            // SEI
            ...
            // STA (Oper,X)
            0x81 => Instruction::STA(Operand::IndirectX(cpu.fetch_byte()), 6),
            // STY Oper
            0x84 => Instruction::STY(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // STA Oper
            0x85 => Instruction::STA(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // STX Oper
            0x86 => Instruction::STX(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // DEY
            ...
            // BCC
            0x90 => Instruction::BCC(Operand::Relative(cpu.fetch_byte() as i8), 2),
            // STA (Oper),Y
            0x91 => Instruction::STA(Operand::IndirectY(cpu.fetch_byte()), 6),
            // STY Oper,X
            0x94 => Instruction::STY(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // STA Oper,X
            0x95 => Instruction::STA(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // STX Oper,Y
            0x96 => Instruction::STX(Operand::ZeroPageY(cpu.fetch_byte()), 4),
            // TYA
            ...
            // LDY #Oper
            0xa0 => Instruction::LDY(Operand::Immediate(cpu.fetch_byte()), 2),
            // LDA (Oper,X)
            0xa1 => Instruction::LDA(Operand::IndirectX(cpu.fetch_byte()), 6),
            // LDX #Oper
            0xa2 => Instruction::LDX(Operand::Immediate(cpu.fetch_byte()), 2),
            // LDY Oper
            0xa4 => Instruction::LDY(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // LDA Oper
            0xa5 => Instruction::LDA(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // LDX Oper
            0xa6 => Instruction::LDX(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // TAY
            0xa8 => Instruction::TAY(2),
            // LDA #Oper
            0xa9 => Instruction::LDA(Operand::Immediate(cpu.fetch_byte()), 2),
            // TAX
            ...
            // BCS
            0xb0 => Instruction::BCS(Operand::Relative(cpu.fetch_byte() as i8), 2),
            // LDA (Oper),Y
            0xb1 => Instruction::LDA(Operand::IndirectY(cpu.fetch_byte()), 5),
            // LDY Oper,X
            0xb4 => Instruction::LDY(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // LDA Oper,X
            0xb5 => Instruction::LDA(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // LDX Oper,Y
            0xb6 => Instruction::LDX(Operand::ZeroPageY(cpu.fetch_byte()), 4),
            // CLV
            ...
            // CPY *Oper
            0xc0 => Instruction::CPY(Operand::Immediate(cpu.fetch_byte()), 2),
            // CMP (Oper,X)
            0xc1 => Instruction::CMP(Operand::IndirectX(cpu.fetch_byte()), 6),
            // CPY Oper
            0xc4 => Instruction::CPY(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // CMP Oper
            0xc5 => Instruction::CMP(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // DEC Oper
            0xc6 => Instruction::DEC(Operand::ZeroPage(cpu.fetch_byte()), 5),
            // INY
            0xc8 => Instruction::INY(2),
            // CMP #Oper
            0xc9 => Instruction::CMP(Operand::Immediate(cpu.fetch_byte()), 2),
            // DEX
            ...
            // BNE
            0xd0 => Instruction::BNE(Operand::Relative(cpu.fetch_byte() as i8), 2),
            // CMP (Oper),Y
            0xd1 => Instruction::CMP(Operand::IndirectY(cpu.fetch_byte()), 5),
            // CMP Oper,X
            0xd5 => Instruction::CMP(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // DEC Oper,X
            0xd6 => Instruction::DEC(Operand::ZeroPageX(cpu.fetch_byte()), 6),
            // CLD
            ...
            // CPX *Oper
            0xe0 => Instruction::CPX(Operand::Immediate(cpu.fetch_byte()), 2),
            // SBC (Oper,X)
            0xe1 => Instruction::SBC(Operand::IndirectX(cpu.fetch_byte()), 6),
            // CPX Oper
            0xe4 => Instruction::CPX(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // SBC Oper
            0xe5 => Instruction::SBC(Operand::ZeroPage(cpu.fetch_byte()), 3),
            // INC Oper
            0xe6 => Instruction::INC(Operand::ZeroPage(cpu.fetch_byte()), 5),
            // INX
            0xe8 => Instruction::INX(2),
            // SBC #Oper
            0xe9 => Instruction::SBC(Operand::Immediate(cpu.fetch_byte()), 2),
            // NOP
            ...
            // BEQ
            0xf0 => Instruction::BEQ(Operand::Relative(cpu.fetch_byte() as i8), 2),
            // SBC (Oper),Y
            0xf1 => Instruction::SBC(Operand::IndirectY(cpu.fetch_byte()), 5),
            // SBC Oper,X
            0xf5 => Instruction::SBC(Operand::ZeroPageX(cpu.fetch_byte()), 4),
            // INC Oper,X
            0xf6 => Instruction::INC(Operand::ZeroPageX(cpu.fetch_byte()), 6),
            // SED
